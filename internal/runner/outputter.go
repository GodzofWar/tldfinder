...
type jsonSourceIPResult struct {
	...
	Whois       *resolve.WhoisInfo `json:"whois,omitempty"`
	Input       string            `json:"input"`
	Source      string            `json:"source"`
}
...
func writePlainHostIP(_ string, results map[string]resolve.Result, writer io.Writer) error {
	...
	for _, result := range results {
		...
		if result.Whois != nil {
			sb.WriteString(",")
			sb.WriteString(result.Whois.Registrar)
			sb.WriteString(",")
			sb.WriteString(result.Whois.CreatedDate)
			sb.WriteString(",")
			sb.WriteString(result.Whois.ExpiryDate)
			sb.WriteString(",")
			sb.WriteString(result.Whois.Registrant)
		}
		sb.WriteString(",")
...
func (o *OutputWriter) WriteHostNoWildcard(input string, results map[string]resolve.Result, writer io.Writer) error {
	...
	for host, result := range results {
		hosts[host] = resolve.HostEntry{Query: host, Host: result.Host, Source: result.Source}
	}
