...
var (
	configDir                     = folderutil.AppConfigDirOrDefault(".", "tldfinder")
...
func ParseOptions() *Options {
	...
	flagSet.CreateGroup("filter", "Filter",
		...
		flagSet.StringSliceVarP(&options.Filter, "filter", "f", nil, " domain or list of domain to filter (file or comma separated)", goflags.FileNormalizedStringSliceOptions),
	)
	...
	flagSet.CreateGroup("update", "Update",
		flagSet.CallbackVarP(GetUpdateCallback(), "update", "up", "update tldfinder to latest version"),
		flagSet.BoolVarP(&options.DisableUpdateCheck, "disable-update-check", "duc", false, "disable automatic tldfinder update check"),
	)
	...
	flagSet.CreateGroup("configuration", "Configuration",
		...
		flagSet.BoolVarP(&options.RemoveWildcard, "active", "nW", false, "display active domains only"),
		flagSet.StringVar(&options.Proxy, "proxy", "", "http proxy to use with tldfinder"),
	...
	flagSet.CreateGroup("debug", "Debug",
		...
		flagSet.BoolVar(&options.Version, "version", false, "show version of tldfinder"),
	...
	if options.Version {
		...
		gologger.Info().Msgf("tldfinder Config Directory: %s", configDir)
		os.Exit(0)
	}
	...
	if !options.DisableUpdateCheck {
		latestVersion, err := updateutils.GetToolVersionCallback("tldfinder", version)()
		if err != nil {
			if options.Verbose {
				gologger.Error().Msgf("tldfinder version check failed: %v", err.Error())
			}
		} else {
			gologger.Info().Msgf("Current tldfinder version %v %v", version, updateutils.GetVersionDescription(version, latestVersion))
		}
	}
...
func ParseDiscoveryMode(value string) source.DiscoveryMode {
	switch value {
	case source.DNSMode.String():
		return source.DNSMode
	case source.TLDMode.String():
